...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        for corner in self.corners:
            if not startingGameState.hasFood(*corner):
                corner_has_food.append(0)
            else:
                corner_has_food.append(1)
        self.StartState = (self.startingPosition,tuple(corner_has_food))
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        for corner_has_food in state[1]:
            if corner_has_food:
                return False
        return True
        #util.raiseNotDefined()
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        cornerHasFood = copy.deepcopy(list(state[1]))
        for i,corner in enumerate(self.corners):
            if cornerHasFood[i] and (nextx,nexty) == corner:
                cornerHasFood[i] = 0
                break
        return ((nextx,nexty),tuple(cornerHasFood))
        #util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    # minDist = 9999
    # pos = state[0]
    # cornerHasFood = state[1]
    # for i,corner in enumerate(corners):
    #     if cornerHasFood[i]:
    #         minDist = min(minDist,abs(pos[0]-corner[0])+abs(pos[1]-corner[1]))
    # return minDist

    # 3.最近食物的曼哈顿距离+唉 我也说不清楚
    minDist = 9999
    pos = state[0]
    cornerHasFood = state[1]
    ...
    for i,corner in enumerate(corners):
        if cornerHasFood[i]:
            minDist = min(minDist,abs(pos[0]-corner[0])+abs(pos[1]-corner[1]))
    if cornerHasFood == (1,1,1,1):
        return minDist + top + right + min(top,right)
    if cornerHasFood == (0,1,1,1) or \
    cornerHasFood == (1,0,1,1) or \
    cornerHasFood == (1,1,0,1) or \
    cornerHasFood == (1,1,1,0) or \
    cornerHasFood == (0,1,1,0) or \
    cornerHasFood == (1,0,0,1):
        return minDist + top + right
    if cornerHasFood == (1,0,1,0) or cornerHasFood == (0,1,0,1):
        return minDist + right
    if cornerHasFood == (1,1,0,0) or cornerHasFood == (0,0,1,1):
        return minDist + top
    return minDist
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        minManhattanDistance = min(minManhattanDistance,abs(position[0]-food[0])+abs(position[1]-food[1]))
    return len(foodGrid.asList())*1000 + minManhattanDistance
    "*** YOUR CODE HERE ***"
    #return 0
